...
func TestEmitTunnel_RequiresAccountID(t *testing.T) {
	...
	if _, err := (TunnelEmitter{}).EmitTunnel(&runtime.Runtime{Cfg: cfg, Providers: runtime.ProviderInputs{Cloudflare: &runtime.CloudflareInputs{}}}); err == nil || !strings.Contains(err.Error(), "CF_ACCOUNT_ID") {
...
func TestEmitTunnel_RendersTunnelResourcesAndLocals(t *testing.T) {
	...
	out, err := TunnelEmitter{}.EmitTunnel(&runtime.Runtime{
		Cfg: cfg,
		Providers: runtime.ProviderInputs{
			Cloudflare: &runtime.CloudflareInputs{AccountID: "acct-id-xyz"},
		},
	})
	if err != nil {
